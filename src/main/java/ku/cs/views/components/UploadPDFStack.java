...
public class UploadPDFStack extends StackPane implements Observer<HashMap<String,String>> {
    ...
    public UploadPDFStack(String dir, String fileName, String curFileName) {
        this.stackPane = this;
        vBox = new VBox();
        hBoxLine1 = new HBox();
        hBoxLine2 = new HBox();
    ...
    protected void handleUploadButtonClick(){
        uploadButton.setOnMouseClicked(e ->{
            curFileName = datasource.uploadFile(fileName);
    ...
    public void saveUploadedImage(){
        if(uploadButtonClicked && curFileName != null && !curFileName.equalsIgnoreCase("no-image")){
            curFileName = datasource.saveFile();
            changeFileLabel(curFileName);
        }
